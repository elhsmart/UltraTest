...
export class GameController {
  ...
  async create(@Body() createGameDto: CreateGameDto) {
    ...
    if (createGameDto.publisherId) {
      publisher = await this.publisherService.findOneById(
        createGameDto.publisherId,
      );

      if (!publisher) {
        throw new NotFoundException(
          `Publisher with id: ${createGameDto.publisherId} not found.`,
        );
      }
    }
    ...
    if (createGameDto.tag) {
      game.tag = await this.tagSevice.findOrCreate(
        createGameDto.tag.split(',').map((tagName) => tagName.trim()),
      );
      await this.gameService.save(game);
    }
  ...
  async getAll(@Request() request) {
    let configPaginationLimit = this.configService.get<number>(
      'PAGINATION_LIMIT',
    )
      ? this.configService.get<number>('PAGINATION_LIMIT')
      : 10;
    let configPaginationOffset = this.configService.get<number>(
      'PAGINATION_OFFSET',
    )
      ? this.configService.get<number>('PAGINATION_OFFSET')
      : 0;

    let result = await this.gameService.findAll({
      limit: request.query.hasOwnProperty('limit')
        ? request.query.limit
        : configPaginationLimit,
      offset: request.query.hasOwnProperty('offset')
        ? request.query.offset
        : configPaginationOffset,
    });

    let presentationResult = [];
    for (let res in result.results) {
      presentationResult.push(
        this.gameMapper.mapFrom(
          result.results[res],
          await this.discountService.findOneByGame(result.results[res].id),
        ),
      );
    }
    let presentation = {
  ...
  async findOne(@Param('id') id: string) {
    ...
    let presentation = this.gameMapper.mapFrom(
      result,
      await this.discountService.findOneByGame(result.id),
    );
    return presentation;
  }
  ...
  async findGamePublisher(@Param('id') id: string) {
    ...
    if (!game.publisher) {
      throw new NotFoundException(
        `Publisher for Game with id :${game.id} not found.`,
      );
    }
  ...
  async update(@Param('id') id: string, @Body() updateGameDto: UpdateGameDto) {
    ...
    if (updateGameDto.tag) {
      game.tag = await this.tagSevice.findOrCreate(
        updateGameDto.tag.split(',').map((tagName) => tagName.trim()),
      );
      await this.gameService.save(game);
    }
