...
export class WorkerProcessor {
  ...
  async handleDiscounts(job: Job) {
    ...
    let dropDateMonthInterval = this.configService.get<number>('GAMES_DROP_MONTHS');
    let discountDateMonthInterval = this.configService.get<number>('GAMES_DISCOUNT_MONTHS');

    this.logger.debug('Checking games older than 12 months');
    try {
        var discountGames = await this.gameRepository.find({
            where: {
                releaseDate: Raw((alias) => `${alias} < (NOW() - INTERVAL ${discountDateMonthInterval} MONTH)`)
            }
        })
    } catch (e) {
        console.log(e);
    ...
    if(discountGames.length > 0) {
        this.logger.debug('Games to be discounted:', discountGames.map(game => game.id+":"+game.title));
    ...
    let deletedGames = await this.gameRepository.find({
        where: {
            releaseDate: Raw((alias) => `${alias} < (NOW() - INTERVAL ${dropDateMonthInterval} MONTH)`)
        },
    });    
    if(deletedGames.length > 0) {
        try {
            this.logger.debug('Games to be removed:', deletedGames.map(game => game.id+":"+game.title));
